...
%
The framework is based on the concept of \textit{provenance templates}, as defined in \cite{Curcin2016} and deals with both stateless transformations, where outputs only depend on the input stream, and stateful ones, outputs also depend on internal state of the activity, and the process may change the state. 

\paragraph{Related work.}

\note{first one this below may not be allowed as the paper is still under submission}

\authornote{Perhaps the closest work is \note{Moreau A Templating System to Generate Provenance}, where templates are formalised in much greater detail than we have done here. However, few details can be found in the paper  on how the bindings are actually generated, whereas we emphasise the \textit{sidecar process} idea for binding generation. Also, none of the example applications deal with stateful transformations of data streams.}

\note{Vasa: can you please add a few words on how each of these differs from our template stuff}
...
The Ariadne system \cite{Glavic2013} provides a series of techniques to capture provenance of data streams, including operators that address various optimization issues, such as minimising latency and computational cost and decoupling provenance computation from stream processing.
...
Note finally, that while our framework produces continuous PROV documents by applying bindings to a template, one can also store the provenance \textit{intensionally} in a database as a stream of bindings along with one (or more) templates, and only generate fragments of the PROV document stream on demand.
