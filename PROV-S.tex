...
\begin{align*}
 ...
 \subsection{Reduce}
Unlike \texttt{map} and \texttt{filter} which are known in Spark as \item , the \texttt{reduce} operator is an \textit{action}, in the sense that it maps a list of elements to a different type of new data structure:
\begin{equation}
...
\authornote{With these distinctions, we describe PROV-S using a progression of:
\begin{description}
